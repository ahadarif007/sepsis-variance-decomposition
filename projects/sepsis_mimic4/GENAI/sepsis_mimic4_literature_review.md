...
**Gap 1: Snapshot and prognosis bias.** Most MIMIC-IV statistical studies use a single baseline measurement or an aggregate score (SOFA, APSIII) and predict mortality after diagnosis, rather than modeling continuously updated, real-time risk of sepsis onset itself. A related aspect of this gap is that no reviewed study explicitly justifies its temporal resolution or prediction mechanism. ICU data is irregularly sampled (vitals every 1–5 minutes, labs every 4–12 hours), yet the choice of aggregation granularity — and whether predictions use a fixed-width sliding window, an expanding window, or a single snapshot — is rarely discussed. The PhysioNet/CinC 2019 Challenge standardized on hourly bins, which balances signal density against computational tractability and aligns with the timescale of clinical decision-making (antibiotics, fluids, and vasopressors are titrated on 30-minute to multi-hour cycles). This implicit hourly convention deserves explicit methodological justification in future work, particularly to distinguish it from per-second waveform-based approaches that would require fundamentally different data sources (continuous bedside monitors) and model architectures (streaming RNNs or transformers).
