...
\begin{table}[H]
    ...
    \begin{tabular}{p{3.2cm}p{9.5cm}}
        ...
        Portfolio, JAMA Network, BMJ Journals (15 sources; see
        \texttt{search\_strategy\_queries.xlsx} for database-specific
        query syntax) \\
        \addlinespace
        Date range &
        ...
        \addlinespace
        Total screened &
        \textbf{TODO: fill in after completing search} \\
        \addlinespace
        Total included &
        \textbf{TODO: fill in after completing search} \\
        \bottomrule
...
Figure~\ref{fig:prisma} summarises the screening process following the
PRISMA 2020 guidelines \cite{page2021prisma}.
...
% --- Identification ---
\node[box] (db) {Records identified through\\database searching (15 sources)\\
  ($n=\textbf{TODO}$ total)};

\node[box, below=of db] (dedup) {Records after duplicates removed\\
  ($n=\textbf{TODO}$)};
\draw[arr] (db) -- (dedup);

\node[excl, right=1cm of dedup] (e1) {Duplicates removed\\($n=\textbf{TODO}$)};
\draw[exarr] (db.east) -- ++(0.5,0) |- (e1.west);

% --- Screening ---
\node[box, below=of dedup] (screen) {Records screened\\(title and abstract)\\
  ($n=\textbf{TODO}$)};
\draw[arr] (dedup) -- (screen);

\node[excl, right=1cm of screen] (e2) {Excluded at screening\\($n=\textbf{TODO}$)};
\draw[exarr] (screen.east) -- (e2.west);

% --- Eligibility ---
\node[box, below=of screen] (elig) {Full-text articles assessed\\for eligibility\\
  ($n=\textbf{TODO}$)};
\draw[arr] (screen) -- (elig);

\node[excl, right=1cm of elig] (e3) {
  Full-text exclusions ($n=\textbf{TODO}$):\\[2pt]
  \begin{minipage}{4.8cm}\raggedright\scriptsize
    Not onset prediction ($n=\textbf{TODO}$)\\
    Paediatric only ($n=\textbf{TODO}$)\\
    No quantitative metric ($n=\textbf{TODO}$)\\
    Editorial/commentary ($n=\textbf{TODO}$)
  \end{minipage}
};
\draw[exarr] (elig.east) -- (e3.west);

% --- Included ---
\node[box, below=of elig, fill=ATUGreen!15, draw=ATUGreen!80!black]
  (incl) {Studies included in review\\($n=\textbf{TODO}$)};
\draw[arr] (elig) -- (incl);

% --- Phase labels ---
\node[phase, left=1.2cm of db]     {Identification};
\node[phase, left=1.2cm of screen] {Screening};
\node[phase, left=1.2cm of elig]   {Eligibility};
\node[phase, left=1.2cm of incl]   {Included};

\end{tikzpicture}
\caption{PRISMA 2020 flow diagram of the literature search and screening
  process. All \textbf{TODO} values should be replaced with actual counts
