...
each stage reads, what it writes, and which result in Chapter~\ref{ch:evaluation}
depends on it. Paths are relative to \texttt{v2-proj/}; all intermediate tables
are Parquet, with a CSV mirror written for any result table small enough to read
directly.

\begin{table}[H]
    \centering\scriptsize\setlength{\tabcolsep}{4pt}
    \begin{tabularx}{\textwidth}{p{0.9cm}p{3.5cm}Xp{2.7cm}}
        \toprule
        \textbf{Stage} & \textbf{Reads} & \textbf{Writes} & \textbf{Supports} \\
        \midrule
        01 & MIMIC-IV source tables &
        --- (validation only; halts on a missing table) &
        Run integrity \\
        \addlinespace
        ...
        Tables~\ref{tab:internal_results}, \ref{tab:utility_optimum},
        \ref{tab:ablation} \\
        \addlinespace
        ...
        Every interval and adjusted $p$-value in this thesis \\
        \bottomrule
    \end{tabularx}
    \caption{Stage inputs, outputs, and the results each supports. Stage 03 is
