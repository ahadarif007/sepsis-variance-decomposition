...
The conventional approach to real-time sepsis prediction seeks to develop a time-varying Cox proportional-hazards model for onset prediction on MIMIC-IV, test the proportional-hazards assumption, benchmark against rule-based scores (SIRS, qSOFA, NEWS), and validate externally on eICU-CRD. An extended literature review of fifty sources establishes that this design, though internally coherent, rests on four premises that the evidence does not support.
...
This study is computationally modest, carries low delivery risk, and — critically — **produces a publishable result whichever way the findings fall**, because its contribution does not depend on the model achieving a performance target. Causal-inference instruments (propensity-score matching, inverse-probability weighting, E-value analysis) are excluded as category errors in a prediction study; group-based trajectory modelling is excluded because it produces no real-time score. The `ricu` package (Bennett, Plečko, Ukor, Meinshausen, & Bühlmann, 2023) supplies a peer-reviewed MIMIC-IV/eICU harmonisation layer and demo datasets requiring no credentialed access. The plan is deliverable within a 22-week MSc timeline.
...
## 1. Introduction: Why the Conventional Approach Is Insufficient

### 1.1 What the conventional approach gets right

The MIMIC-IV statistical literature is dominated by post-diagnosis prognostic modelling rather than pre-onset detection; the proportional-hazards assumption is not reported as tested despite Kasal et al.'s (2004) demonstration that it frequently fails in sepsis cohorts; and external validation is scarce precisely where it matters most. A currency scan of the 2025–2026 MIMIC-IV literature confirms the first of these claims: every located study published in that window predicts an outcome *after* sepsis has been diagnosed. These are genuine gaps.

Interpretability, assumption-testing, and external validation are non-negotiable design commitments. Those are retained here in full. What changes is the *object* to which they are applied.

### 1.2 What the conventional approach gets wrong

The standard prescription — build a time-varying Cox model for sepsis onset on MIMIC-IV, and benchmark against rule-based scores — encounters four independent difficulties, each documented in the extended review and each sufficient on its own to compromise the contribution.

**(i) The gap is narrower than often claimed.** The assertion that statistical onset modelling "has not been substantially revisited on MIMIC-IV specifically" since Henry et al.'s (2015) work on MIMIC-II is inaccurate. Fagerström et al. (2019) fitted a Cox proportional-hazards model on MIMIC-III, using Henry et al.'s exact input variables and exact septic-shock target, as a head-to-head baseline — and an LSTM beat it by up to twenty hours of lead time. The gap that remains is not "nobody has fitted this model" but "nobody has fitted a properly specified, label-sensitivity-analysed, temporally validated survival model for onset on MIMIC-IV." That is still a real gap. It is simply a more demanding one, and it is not closed by fitting the model Fagerström et al. already fitted and lost with.

**(ii) The benchmark is weak.** Benchmarking against SIRS, qSOFA, and NEWS is insufficient. The Surviving Sepsis Campaign guidelines (Evans et al., 2021) issue a **strong recommendation against using qSOFA as a sole screening tool**, on grounds of poor sensitivity. Demonstrating superiority over an instrument that the governing clinical guideline has already told practitioners not to use for screening is not a persuasive result. Furthermore, Mellhammar et al. (2020) already constructed a LASSO-derived statistical score and **failed to beat NEWS2** — a direct precedent that demands NEWS2 as the minimum meaningful comparator.

**(iii) The validation design is optimistically biased for this outcome specifically.** Internal validation via a random train/test split within MIMIC-IV is problematic. Guo et al. (2022) partitioned MIMIC-IV by admission-year group and evaluated four prediction tasks — mortality, long length of stay, sepsis, and invasive ventilation — under temporal shift. Sepsis was the **worst-affected task**, and domain-generalisation and domain-adaptation algorithms failed to outperform plain empirical risk minimisation. A random split on MIMIC-IV therefore inflates apparent performance more for sepsis than for any neighbouring outcome a reader might use to calibrate their scepticism.

**(iv) The evaluation metrics cannot see the failure.** Nominating AUC-ROC, sensitivity, specificity, and lead time as primary metrics is inadequate. Wang et al. (2025), reviewing ninety-one real-time sepsis prediction models, report that these are precisely the quantities that survive the internal-to-external transition while clinical value inverts: median AUROC 0.811 → 0.783 (not statistically significant), median Utility Score **+0.381 → −0.164**. A study reporting those metrics and concluding that its model generalises would have failed to measure the thing that fails. The same pattern appears in deployment: Wong et al. (2026) report AUROC between 0.82 and 0.92 for the Epic Sepsis Model version 2 across 227,091 encounters, *alongside* low positive predictive value and high alert burden.

### 1.3 The core inversion

The conventional approach treats the label, the split, and the metric as **nuisance parameters** — matters of convention to be fixed so that the model can be evaluated. The literature says the opposite. Cohen et al. (2024) show that the label moves performance at least as much as the model does. Guo et al. (2022) show that the split determines whether the performance estimate is honest. Wang et al. (2025) show that the metric determines whether a collapse is visible at all.

**The variance lives in the analyst's choices, not in the model.** This inversion is the contribution of this study. It converts the project from a ninety-second entrant into a crowded field — where the ninety-first entrant's rank on a leaderboard carries, per Reyna et al. (2020), a Spearman correlation of approximately −0.03 to 0.01 with its performance at an unseen hospital — into a methodological study that the field measurably needs and that has not been performed for a survival model on MIMIC-IV.
...
→ **Design consequence:** framing must be **pre-specified**, and coefficient stability across framings must be reported. This is the source of hypothesis H2. It is also the direct threat to the conventional approach's interpretability premise: a hazard ratio whose sign is an artefact of framing is not an interpretable quantity.
...
→ **Design consequence:** the conventional approach does not engage this dispute at all. this study must, and the discrete-time formulation is the resolution: it is a **cause-specific competing-risks model** (satisfying Resche-Rigon et al.) that is *also* **a logistic regression** (satisfying Schoenfeld), while retaining the time-to-event structure Schoenfeld would discard. The dispute is answered by construction rather than by choosing a side.
...
→ **Design consequence:** landmarking is *not* an escape from the conventional approach's proportional-hazards problem; it is a different model with its own approximation error. The discrete-time formulation is preferred, with landmarking retained as the mechanism for allowing coefficients to vary with time-in-ICU.
...
→ **Design consequence:** retained from the conventional approach as the external cohort. Correct choice, unchanged.
...
→ **Design consequence:** supplies the net-benefit analysis. Retained from the conventional approach, now correctly positioned as a primary rather than supplementary analysis.
...
→ **Design consequence:** a direct precedent for the conventional approach's plan, and a negative one. Beating NEWS2 is *not* assumed to be achievable, and the study is not designed such that failing to beat it constitutes failure.
...
→ **Design consequence:** supplies the performance reference range. Note that AISE is itself a **modified Weibull–Cox** formulation — the leading "machine-learning" sepsis model of its generation has a survival-analytic backbone, which further dissolves the statistics-versus-ML dichotomy the conventional approach relies on.
...
## 3. Comparative Analysis: Conventional Approach versus This Study

### 3.1 Head-to-head design comparison

| Design element | Conventional approach | This study | Warrant for the change |
|---|---|---|---|
...
Three components of the conventional approach are removed. Two of these are removals of *category errors*, and stating them plainly is more useful than politely retaining them.
...
the conventional approach inherits these tools from the base literature review's Theme 4 (Guo et al., 2025; Zou et al., 2022) — a body of work that uses them entirely appropriately, because those studies are asking **aetiological questions** ("does heparin reduce sepsis mortality?"). The import into a prediction proposal is a straightforward confusion of estimands. Removing them is not a scope reduction; it is a correction, and it frees Weeks 13–14 of the conventional approach's schedule.

**(b) Group-based trajectory modelling. Removed (may be reinstated as a purely descriptive appendix if time permits).**
GBTM is a **latent-class clustering method**. It partitions patients into trajectory groups *post hoc*, using the full observed trajectory — including data from after the prediction time. It does not produce a real-time risk score, it cannot be evaluated on the Utility Score, and its groups cannot be assigned prospectively without a separate classifier. the conventional approach lists it as an exploratory extension and simultaneously (correctly) flags in risk R7 that it may consume disproportionate time. The resolution is to remove it. Sepsis heterogeneity is real and important (Seymour et al., 2019), but it is addressed here through **subgroup analysis** (§5.10) and through the flexible baseline hazard, not through a clustering method that answers a different question.
...
### 3.4 The effort ledger: why this study is not a larger project

A supervisor's first and entirely reasonable objection will be that this study looks more ambitious. The ledger says otherwise.

| Removed from the conventional approach | Added in this study |
|---|---|
...
| Bespoke MIMIC-IV → eICU variable mapping (the conventional approach risk R6, Med/High) | `ricu` harmonisation layer (Bennett et al., 2023) |
...
**Net assessment.** The removals are computationally and conceptually heavier than the additions. this study is **comparable in total effort, materially lower in delivery risk, and substantially higher in contribution**. The two heaviest risk items in the conventional approach — credentialing delay (R1) and cross-database schema mismatch (R6), both rated Medium likelihood with High impact — are both largely retired by a single dependency (`ricu`), which supplies demo datasets requiring no credentialed access and a peer-reviewed harmonisation layer covering both target databases.
...
**Hourly binning**, following the convention established by the PhysioNet/CinC 2019 Challenge (Reyna et al., 2020) and AISE (Nemati et al., 2018). the conventional approach's justification of hourly resolution — that vitals are charted on minute-to-hour timescales, labs on 4–12 hour timescales, and clinical interventions titrated on 30-minute-to-multi-hour cycles, so that sub-hourly prediction is clinically redundant and would require waveform data the databases do not contain — is **retained in full**. It was one of the strongest passages in the conventional approach and remains correct.
...
3. **Static Cox proportional-hazards model** — the field's default, retained *as a deliberately weak baseline*, with formal Schoenfeld-residual testing. Its purpose is to answer the question the conventional approach could not: *what does the default choice actually cost?*
...
| **Internal (secondary)** | Random patient-level split | Reported *solely* to quantify the optimism of the design the conventional approach proposed |
...
If the conventional approach's implicit premise is correct, **model class dominates**. If the literature reviewed in §2 is correct, it does not. **That table is the dissertation.**
...
**A structural feature worth noting:** the first reportable result (M1, the label-variance table) lands in **Week 7**. the conventional approach's first substantive result — baseline models — landed in Week 8, and its *contribution-bearing* result, the time-varying model, in Week 12. this study therefore front-loads its contribution, which materially reduces the risk of a late-stage failure leaving the dissertation without a finding.
...
| # | Risk | L | I | Mitigation | Change vs. the conventional approach |
|---|---|---|---|---|---|
...
| **R2** | MIMIC-IV ↔ eICU schema/variable mismatch | **Low** | Med | `ricu` supplies a peer-reviewed harmonisation layer covering both, handling the absolute-vs-relative timestamp problem; used by Moor et al. (2023) for exactly this purpose. Fallback at G4. | **Likelihood downgraded** from Med; this was the conventional approach's single largest risk |
...
**The most important line in this table is R8.** In the conventional approach, the outcome "the time-varying Cox model does not outperform the static baseline" was a Medium/Medium risk requiring active mitigation — which is to say, the conventional approach's success was contingent on a performance result it could not guarantee, in a field where ninety-one prior models exist and leaderboard rank does not transfer out of distribution. In this study, that same outcome is a pre-registered hypothesis whose confirmation and whose rejection are *equally publishable*. **The project cannot fail on performance grounds, because performance is not the contribution.** This is the single strongest argument for the revision.
...
**It is not a larger project.** The removals — the time-varying Cox partial likelihood on hourly data, group-based trajectory modelling, and the propensity-score/inverse-probability-weighting/E-value apparatus (which are causal-inference instruments with no role in a prediction study) — are heavier than the additions. `ricu` retires the two largest risks the conventional approach carried.

**It cannot fail on performance grounds.** the conventional approach's risk R3 — "the time-varying Cox model does not outperform the static baseline" — was a genuine threat to the dissertation. In this study that same outcome is pre-registered hypothesis H6, and its rejection is as publishable as its confirmation. The first reportable result arrives in Week 7.

**It answers a question the field's strongest recent papers explicitly leave open.** Cohen et al. (2024) did the label half on MIMIC-III with generic models. Guo et al. (2022) did the temporal half on MIMIC-IV without varying the label. Wang et al. (2025) documented the metric divergence across ninety-one studies without controlling any of it. Nobody has held the model fixed and measured how much everything else was doing — and the tools of classical statistics, which the conventional approach was right to want to use, are unusually well suited to precisely that task.

the conventional approach's instinct was sound. Statistics *does* have something to offer here. The offer is simply not another model. It is the discipline to find out what the models have been measuring.
