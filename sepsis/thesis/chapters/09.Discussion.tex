...
H6 is \emph{not} rejected: the two flexible estimators differ by \pcHSixEst{}
AUROC, and the interval's upper limit sits inside the 0.02 bound, so the
boosted trees are not better by more than the margin. H1 is also not
...
gradient-boosted trees, and is marginally ahead on point estimates. That is not
...
\begin{enumerate}
    ...
    the 0.02 margin (Section~\ref{sec:h6}). Neither gap affects the structural
    ...
    \item \textbf{Seven protocol amendments are post-hoc.} The inference layer
    ...
    floor (Section~\ref{sec:equity_strata}) and the H2 uncertainty diagnostic
    (Section~\ref{sec:h2_uncertainty}) were all specified after the
    ...
    error-control plan, and the amendment results as exploratory. Three of the
    seven warrant a specific note, and they do not all point the same way.
