...
was necessary. On point estimates alone, H1 would have been read as confirmed
and the study would have told a tidy story in which the label dominates
discrimination. With intervals attached that story is not supportable: the
label spread is \pcSpreadLabel{} [\pcSpreadLabelCiLo, \pcSpreadLabelCiHi]
against a model-class spread of \pcSpreadModelClass{}
[\pcSpreadModelClassCiLo, \pcSpreadModelClassCiHi], and the two intervals do
not overlap. The two sources are not of comparable size, and the defensible
...
the two flexible estimators are indistinguishable inside the development
cohort, but only one of them stays ahead of a bedside score outside it, and the
one that does is the interpretable model. And it cuts against this study's own
...
\begin{enumerate}
    ...
    calibrated to reach it. The metric-choice finding (H5) rests on the gap
    between discrimination and achievable utility, and these three orderings
    are what that gap looks like when it is written out.
\end{enumerate}
...
\pcHSixAbsEst{} AUROC, so a study comparing one flexible model against another is
...
\begin{enumerate}
    ...
    microbiology recording matches MIMIC-IV's.
    ...
    \item \textbf{Discrimination is measured on the current-hour hazard, not on
    a six-hour horizon.} The models estimate the cause-specific hazard of onset
    ...
    rates. And a cumulative six-hour risk, which is what a deployed instrument
    ...
    \item \textbf{Eight protocol amendments are post-hoc.} The inference layer
    ...
    (Section~\ref{sec:h2_uncertainty}) and the comparator fit isolation
    (Section~\ref{sec:gbt_spec}) were all specified after the
    ...
    error-control plan, and the amendment results as exploratory. Four of the
    eight warrant a specific note, and they do not all point the same way.
    ...
    count alone and is blind to outcome, because the reduction removed the
    family's only surviving discovery rather than creating one, and because
    nothing in E1 survives correction under either denominator. A reader who
    rejects that reasoning may read the thirty-contrast correction instead and
    ...
    Amendment 8 is the only one of the eight that changes a reported estimate
    ...
    For H1, H4 and H6 the correction determines nothing: their adjusted
    $p$-values are \pcHOnePHolm, \pcHFourPHolm{} and \pcHSixPHolm, and each
    fails on its point estimate too. H5 is the exception on precision (its adjusted value is \pcHFivePHolm, and it fails
    on the width of its interval rather than on its direction), which is
    precisely why the substitution needed disclosing rather than absorbing.
\end{enumerate}
