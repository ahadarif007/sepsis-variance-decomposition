...
identical stays it contains \pcPreTreatOnsetsVarF{} of them, separable from
...
not to mistake it for a Sepsis-3 result. The treatment-independent arm of
...
\textbf{The external event-rate gap is a property of the anchor, not of the
population.} The Sepsis-3 arm reaches only \pcExtNMicroStays{} of
...
anchored label gave \pcExtRateRatioB. The shortfall largely disappears when the
...
internally, and it is not comparable with \pcExtAurocPrimaryB. What the event
count does support is the \emph{drop}: internal to external degradation of this
order is no longer an inference from \pcExtNEventsB{} events.
