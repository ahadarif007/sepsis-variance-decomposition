...
(\texttt{00\_preregistration.md}, locked before any of the modelling reported
here) and reports
according to TRIPOD+AI \cite{collins2024tripodai} and PROBAST+AI
\cite{moons2025probastai} guidelines. The design treats the label definition,
validation split and evaluation metric as primary variables rather than as
fixed background choices.

All code is implemented in R (4.5.2) and Python (3.14, \texttt{pandas} 3.0.4),
which are the versions the reported results were produced under; Appendix~\ref{app:repro} records the
minimum supported versions and the dependency pins. The pipeline is driven by a single
...
\adjustbox{max width=\textwidth}{%
    ...
    adult admissions only, so no minor is present to remove. The filter is run
    rather than assumed, and its count is generated like every other. Sepsis counts are \textbf{cohort-wide},
...
at any point: Variant D is the anchor-independent labelling that the
...
The term needs bounding, because it is easy to read as more than it is.
``Anchor-independent'' means independent of the treatment \emph{timestamps}
that anchor Sepsis-3: no antibiotic, culture or vasopressor event contributes
to membership or to onset timing. The label is called anchor-independent rather
than treatment-independent throughout this thesis for exactly that reason. It
does not mean independent of clinical care in any broader sense. Variant D is built from measurements that exist because
somebody ordered them, on patients whose physiology already reflects whatever
treatment they had received, and recorded at a frequency that itself tracks how
closely a patient is being watched. Anchor-independence is what the comparison
needs and what it has; causal independence from treatment is neither claimed
nor constructible from routinely collected ICU data.

The PhysioNet/CinC 2019 operationalisation \cite{reyna2020utility} was
considered as the \emph{anchor-independent} alternative label and rejected
...
Variant F was added for that reason and only that reason. The pre-registered
onset rule was found to make H3 unfalsifiable, so a variant carrying the
standard timing was needed for the question to be answerable at all; the
amendment was fixed before Variant F was fitted, and nothing about its
performance entered the decision to add it.
...
Variant B's, so the two variants label exactly the same stays cohort-wide; only
the timestamp moves, and only ever earlier. Variant F therefore isolates the timing
...
free of treatment in general. Variant D remains the anchor-independent
comparison, within the bound stated above.
...
\emph{shift}, not the overlap. Agreement is still not perfect once
...
The alerting window and the utility scoring rule are adapted from the
PhysioNet/CinC 2019 Challenge \cite{reyna2020utility} rather than reproduced
from it, and Section~\ref{sec:metrics} states the adaptation in full. The
estimand differs too, the Challenge having scored a binary label defined over
the window itself.
...
Penalisation here is a precondition rather than a refinement. Coefficient
...
An XGBoost model is fitted under the same conditions for each label variant and
split. Settings: logistic objective, area under the precision-recall curve as
the evaluation metric, maximum depth 6, learning rate 0.05, row and column
subsampling at 0.8, minimum child weight 5, and positive-class weighting by the
observed class ratio. None of these is tuned; they are the library's
conventional starting values, fixed once and applied identically to every
variant, split and feature arm. Tuning the comparator would confound the
model-class contrast with a difference in search budget, which is the same
objection that keeps the ablated arm on the full arm's hyperparameters
(Section~\ref{sec:ablation_method}).

The number of boosting rounds is the one quantity that is chosen rather than
fixed, and it is chosen by early stopping, which needs a set the model is not
fitted on. That set is carved out of the \emph{training} rows: 15 per cent of
training stays are held out, the model is fitted on the remainder, and boosting
stops after 30 rounds without improvement on the holdout. The split is on the
stay rather than the person-hour, because person-hours are roughly forty-five
correlated rows per stay and a row-level split would leave the same patient on
both sides of it. The test set is never used to select the number of rounds
(Protocol Amendment 8).

\subsection{Comparator: Cox Proportional-Hazards Model}
\label{sec:cox_spec}

A Cox proportional-hazards model, included as the field's default
survival specification and as a deliberately weak baseline. The
pre-registration names it the \emph{static} comparator, and the sense in which
it is static is worth stating exactly, because it is not the inputs. The model
is fitted on the same training person-hours as every other comparator, in
counting-process form: each hour contributes the interval
$(t,\, t+1]$ with the event indicator of that hour, so the covariates entering
it are the same hourly, forward-filled values the primary model sees. What is
static is the \emph{coefficient} structure. Each covariate is given one
time-constant log hazard ratio, the baseline hazard is left unspecified rather
than modelled, and no interaction with elapsed time is estimated. That is the
assumption Section~\ref{sec:schoenfeld} tests and rejects.

The specification is otherwise restricted relative to the primary model, which
is what makes it the weaker baseline:
...
\begin{itemize}
    ...
    order (\emph{measured}) indicators are \emph{not} included, so the
    comparator sees levels but neither trends nor the record of clinical
    attention.
    ...
    discharge and death as censoring rather than as competing causes; that
    difference is part of what the comparison is for.
    \item \textbf{Ties.} Efron's approximation, which is the relevant choice
    here because hourly binning makes ties the rule rather than the exception.
    \item \textbf{Uncertainty.} Standard errors are clustered on the stay, so
    the repeated hours a patient contributes are not treated as independent.
...
Schoenfeld residual testing is applied to verify the expected violation of
...
An \textbf{adapted PhysioNet-style Utility Score} is the primary metric. It
rewards early true alerts and penalises late and false ones, following the
design of the PhysioNet/CinC 2019 Challenge score \cite{reyna2020utility}.
AUROC is reported only for comparison, because it hides the clinical failure
mode \cite{wang2025srpm}.

\paragraph{Where it departs from the Challenge score, and why.} The
scoring rule used here is not the Challenge's own implementation and is not
interchangeable with it, so scores reported in this thesis are not comparable
with a published Challenge score. Three departures matter. The Challenge
accumulates utility at \emph{every} time step of every record; the rule here
accumulates it \emph{per stay}, crediting the earliest qualifying alert once
rather than each alerting hour, so that a model is not rewarded for holding an
alarm on. The reward window is the six hours this study pre-registered as its
alerting window, rather than the Challenge's wider ramp beginning further ahead
of onset, which keeps the metric aligned with the horizon used everywhere else
in this design. And the penalty for never alerting on a septic stay is fixed at
$-1$ per stay rather than accumulated over the post-onset hours. The
normalisation against the inaction and optimal strategies is the Challenge's
and is the part that carries the metric's meaning, which is why the score
retains its reading of ``0 is exactly the no-alert strategy''. The parameters
are pre-registered constants, not tuned quantities, and they are fixed in
\texttt{config.R} for every model and variant alike, so the comparisons made
\emph{within} this thesis are unaffected by the adaptation.
...
\cite{vancalster2019calibration}, is also reported as a flexible
...
The direction is exculpatory: under the thirty-contrast denominator exactly one
contrast fell below the Benjamini--Hochberg threshold, a language contrast
resting on twelve events, and the floor removed it. A family narrowed to
manufacture significance narrows \emph{around} its significant results; this
one discarded the only one it had. And the verdict is invariant for every
contrast the reduced family retains: none of the
\pcNSubgroupContrasts{} clears Benjamini--Hochberg under either denominator, the
smallest adjusted value being $q = \pcSubgroupMinQ$ at
$m = \pcNSubgroupContrasts$ and larger still at $m = 30$. The amendment
therefore changes which contrasts are read and, in the one case where it
changes a verdict, changes it against this study.
...
The full protocol was locked before the cohort extraction reported here. All six hypotheses,
...
is labelled exploratory or descriptive, according to whether it carries an
inferential claim, and, following
