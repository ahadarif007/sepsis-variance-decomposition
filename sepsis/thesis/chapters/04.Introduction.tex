...
``Which model is best?'' is therefore the wrong question to lead with. The
question this thesis asks instead is \textit{how much of a model's reported
performance comes from the model itself, and how much comes from how the study
was designed: how sepsis was defined, how data was split, and which metric was
reported?}
...
One scoping statement belongs with the question rather than after it, because
it governs how every number in this thesis should be read. The models estimate
the hazard of sepsis onset in the \emph{current} hour, and every AUROC reported
here scores that hazard against an indicator of the onset hour among the hours
in which a patient is still at risk. That is detection at the recorded onset,
not anticipation of it over a future horizon, and the timing is exact only to
the hour: predictors and the onset are binned the same way, so within that one
hour a measurement charted after the onset timestamp still enters the
prediction assigned to it. ``Real-time'' here therefore describes the
estimand's form, a hazard evaluated at every hour of the stay with no
retrospective information, and not a demonstrated lead over the clinician.
Where this thesis reports that events exist before treatment and are separable
from their surroundings, it is reporting exactly that and nothing about advance
warning; Limitation~\ref{lim:current_hour} states the consequence in full.
...
\begin{description}
    ...
    moving from internal to external validation. This proved not to be evaluable,
    for the reasons given in Section~\ref{sec:h5}: there is no internal Utility
    Score for a decline to be proportional to. A substituted
    internal-to-external AUROC contrast was analysed in its place and retains
    H5's slot in the multiplicity correction.
    ...
    pre-registered statement is two-sided, so its null is an interval with two
    boundaries; both are tested and neither is cleared, as
    Section~\ref{sec:h6} sets out.
\end{description}

H1 and H6 pull against each other by design. If H1 is confirmed, the label is
the main source of variation. Rejecting H6 would instead put weight back on the
model, and there is no outcome here that leaves the question unanswered.
...
\begin{enumerate}
    ...
    $\min(t_{\text{susp}}, t_{\text{SOFA}})$ rule applied to the same stays,
    whose septic membership it leaves cohort-wide unchanged,
    recovers \pcPreTreatOnsetsVarF{} pre-treatment onsets, separable from
    their surrounding at-risk hours at \pcPreTreatAurocPrimaryF{} AUROC on the
    onset-hour estimand. Kamran et al.'s
    ...
    hour, while an anchor-independent deterioration label agrees with
    Sepsis-3 at $\kappa = \pcKappaVsBD$, close to chance. And the treatment
    anchor alone is about as predictable as the Sepsis-3 conjunction built on
    top of it, so the difficulty lies not in the conjunction but in when the
    label is stamped. That second result is model-dependent and is reported as
    such: the anchor-attributable fraction is
    $\phi_{\text{anchor}} = \pcPhiAnchorPrimary$
    [\pcPhiAnchorCiLoPrimary, \pcPhiAnchorCiHiPrimary] for the primary model,
    an interval including 1, and $\pcPhiAnchorGbt$
    [\pcPhiAnchorCiLoGbt, \pcPhiAnchorCiHiGbt] for the gradient-boosted
    comparator, which excludes it. The same
    comparison shows the empty pre-treatment window of C2 to be a property of
    the onset rule, not of the patients: an anchor-independent label records
...
The data, the labels, the models and the evaluation plan are set out in
Chapter~\ref{ch:methodology}, and Chapter~\ref{ch:design} explains the pipeline
architecture and its twelve scripts.
Chapter~\ref{ch:evaluation} presents all results, organised around the six
hypotheses. Chapter~\ref{ch:discussion} interprets them, bounds what each
estimate can support, and states the limitations, before
