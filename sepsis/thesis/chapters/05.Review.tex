...
does not carve the problem at its joints. Next comes the outcome itself: what
counts as sepsis onset, and whether it is a stable enough object to support the
interpretable effect estimates that motivate statistical modelling
(Sections~\ref{sec:label_object} and~\ref{sec:label_action}). Validation
supplies the third: what the published external-validation record actually
shows, and why discrimination and clinical utility tell opposite stories about
it (Sections~\ref{sec:generalisation} and~\ref{sec:auroc_illusion}). The
last is methodological, and it is genuinely open in the statistical literature
rather than merely under-reported. No survival formulation for ICU event data
satisfies all of its own assumptions, and Section~\ref{sec:survival_dispute}
takes a position on which set of violations is acceptable.
...
matches machine learning''. The equivalence is established for static risk
stratification and contested for the temporal early-warning
...
deployment, at $n \approx 200{,}000$, with a commercial model. The finding
holds across dataset, scale, model class and setting.
...
\begin{figure}[H]
    ...
    cohorts, and the regions are the point: only the central region is septic
    under all three readings, while each crescent holds patients whom one
    reading counts and the other two do not. The disagreement is larger than
    the three prevalences separately suggest, which is the effect Cohen et al.\
    \cite{cohen2024subtle} report on MIMIC-III and which the ranges quoted
    above summarise. The corresponding measurement on MIMIC-IV is reported in
    Chapter~\ref{ch:evaluation}, where the two narrowest readings agree at
    only $\kappa = \pcKappaVsBA$.}
...
Kamran et al.\ \cite{kamran2024evaluation} pursued this furthest, and their
result bears on the validity of the prediction task itself rather than on any
one model's performance within it. Clinicians frequently recognise
...
still be clinically unusable.

Taken together these two results constrain the argument for interpretable
models from both sides. If framing can flip the sign of a coefficient, then the
...
$\rightarrow$ 0.783. It is a reference estimate drawn from one multi-database
study and used here for planning, not an established typical value, and the
distinction matters because the whole external design is sized against it. The
number is specific enough to fix an expectation in advance, which is what this
study's external arm is calibrated to; H5 itself asks a different question,
whether the Utility Score declines proportionally further than AUROC does, and
0.085 bounds only the AUROC limb of that comparison. A study observing a
substantially \emph{smaller} drop should first suspect that its external cohort
is insufficiently distant rather than conclude that its model is unusually
stable.
...
additions to that list; they are the list. This is why the Utility Score is
...
Its properties answer the dispute point by point. The formulation is native to
the data's structure, because ICU data is already binned hourly by convention
...
scale and the deployment's is required. Proportional hazards is not required,
so item (1) is dissolved rather than tested. Competing risks are
accommodated directly through a multinomial cause-specific formulation over the
...
answers item (2) without incurring item (3). Schoenfeld's objection in item (3)
is met on its own terms, since the model is a logistic regression, and the
time-to-event structure he would discard is retained. Its coefficients are
log-odds of onset in the next hour given survival sepsis-free to this hour,
arguably a more clinically legible quantity than a hazard ratio. And it emits
exactly the
...
genuinely interpretable quantity. One that \emph{flips} across those readings is
a negative result worth reporting. Either way the analysis is informative, and
it is an analysis this search did not find performed for a survival model on
MIMIC-IV.
...
been demonstrated separately, by different groups working in different
databases with different models. What has not been done is to hold one database
and one pipeline fixed and ask how much of the reported performance each of
these analyst decisions accounts for, relative to the choice of model that the
literature spends most of its effort on. That question is answerable, and
answerable with the intervals the constituent studies lack. It is what this
thesis does.
