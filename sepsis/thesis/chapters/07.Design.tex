...
\texttt{sepsis/\allowbreak project/}, preceded by the locked pre-registration
document \texttt{00\_\allowbreak preregistration.md}. Each script reads from
upstream outputs and writes its own results to
\texttt{sepsis/\allowbreak project/\allowbreak output/}. No script modifies the
...
source of truth for every path, identifier, window, threshold, seed, family
size, event floor and covariate vector in the pipeline, and no other file
hard-codes any of them. The covariate vector is the case that matters most and
was the last to be brought inside the rule: it is read by the two fitting
stages and again by the external-validation stage, which reconstructs a stored
model's column order from it, so a divergence between copies would have
produced a permuted design matrix and predictions that are wrong while
remaining perfectly well-formed. Where a stored model records its own feature
names, disagreeing with the declaration is an error rather than a warning. The
...
A crash is not the pipeline's most dangerous failure mode. That would be a
stage that quietly does less than it was asked to. Selecting predictors by
intersecting a
...
\begin{enumerate}
    ...
    chartevents, labevents and inputevents. A SOFA rise
...
The gradient-boosted comparator is also fitted on the ablated feature
...
intercept and slope, the adapted PhysioNet-style Utility Score of
Section~\ref{sec:metrics}, alert burden, lead time, and
...
Three arms are reported. The primary arm applies the Sepsis-3 conjunction to
the microbiology-covered sub-cohort; the second applies an antibiotic-only
...
The third arm is Protocol Amendment 9 and is described here because nothing
about it is inferable from the first two. It applies the frozen Variant D
models (the same \texttt{05\_}/\texttt{06\_} objects the other arms use,
loaded without retraining) to the whole eICU-CRD cohort rather than to the
microbiology-covered sub-cohort, which is possible because Variant D's onset
rule reads physiology alone and needs no culture record. The external panel is
the one already built for the other arms, so the feature construction, the
plausibility ranges and the last-observation-carried-forward rule are shared;
harmonisation is the same \texttt{check\_model\_features()} path, which
reconstructs the design matrix from the declared feature vector and errors
rather than silently reordering it. The one substantive difference is in the
label, not the features: eICU's \texttt{vitalPeriodic} carries no Glasgow Coma
Scale, so the external SOFA delta rests on five components against MIMIC-IV's
six (Section~\ref{sec:external_altlabel}). Results go to
\texttt{08\_external\_validation\_results\_altlabel} and
\texttt{08\_event\_rate\_altlabel}, and the per-hour predictions to
\texttt{08\_eicu\_predictions\_D}, which is what stage 12 bootstraps the
transport interval from under Amendment 10. The arm writes to no file the
pre-registered arms read.
...
produced by a label that was never varied. Two further guards keep the arms
apart: the Amendment 9 file may not contain a pre-registered variant, and the
pre-registered file may not contain a post-hoc one, so a post-hoc label cannot
enter H5 through the external door after being excluded from H1.
...
\begin{itemize}
    ...
    \item \textbf{Metric divergence (H5):} the pre-registered contrast between
    the AUROC and Utility Score declines is attempted, found to be undefined
    against an internal Utility Score indistinguishable from zero, and recorded
    as such; the substituted internal-minus-external AUROC contrast is written
    to its own row (Section~\ref{sec:h5}).
\end{itemize}
...
carrying at least \texttt{MIN\_\allowbreak SUBGROUP\_\allowbreak EVENTS\_\allowbreak INTERPRET} sepsis onsets enter
...
for every headline and subgroup estimate; the Holm-corrected confirmatory family
for H1--H6, which contains formal tests where one is defined, descriptive or
definitional records for H2 and H3, and the separately identified contrast
substituted for H5, alongside the Benjamini--Hochberg-corrected exploratory
families (E1 and E2 subgroup analyses, E3 alternative-label contrasts); and the machine-readable
...
The pipeline is orchestrated by \texttt{run\_\allowbreak pipeline.R} (or the equivalent
\texttt{run\_\allowbreak pipeline.sh}), which executes all stages sequentially or by phase,
...
(compressed, columnar). Small result tables ($<$1,000 rows) are also
...
All random operations use a single fixed seed (\texttt{42}) to ensure
...
Seeding fixes the pseudo-random draws; it does not by itself make a result
bitwise reproducible across environments. Within one environment every estimate
here is intended to reproduce, and the train/test partition is invariant by
construction because it is defined by admission year rather than by sampling.
Across differing library builds, thread counts or architectures the
gradient-boosted comparator is the one step that need not reproduce to the last
bit, which is why its rows are designated the consistency check on any re-run
...
re-run the pipeline. Within the pinned analysis environment and with the fixed
seed above, it reproduces the reported estimates; the gradient-boosted
comparator is the one output that may vary slightly across library builds or
architectures.
