...
operational criteria, NEWS2, qSOFA, SIRS, the adapted PhysioNet-style Utility
Score, the
...
\section{The Frozen Run}
\label{app:frozen_run}

Every quantity in this thesis is written mechanically from the pipeline's result
tables by \texttt{thesis\_constants.R}, so no current estimate is transcribed by
hand and every such value in the text agrees with the table it is drawn from.
The one deliberate exception is named below: the two \emph{superseded} figures
this section prints in order to compare them against the current ones. That is
a guarantee about transcription and about nothing else: it cannot establish that
the right output cell was selected for a sentence, that the sentence still
describes what the value shows, or that a claim has not outlived the analysis it
was written for. Those remain human obligations, and
Section~\ref{app:repro} describes the checks that close part of the gap.
Whether the document is consistent across \emph{runs} is a separate question
again, and it is worth answering with a measurement rather than an assumption.

The run this document is bound to was executed end to end for that purpose, and
it reproduced \emph{every} reported quantity exactly: of the macros
\texttt{thesis\_constants.R} emits, the only ones that changed were the
provenance stamp and the generation timestamp. On a fixed machine, R version and
library build, with the seeds in \texttt{config.R} and a partition defined by
admission year rather than by sampling, this pipeline is reproducible to the
last digit it prints.

Two qualifications keep that from being a stronger claim than it is. The
gradient-boosted comparator is the one component with no guarantee of
reproducibility across \emph{differing} library builds or architectures, so the
statement above is about re-running this environment and not about rebuilding it
elsewhere. And one intermediate artefact is known to vary between runs of
identical source: the per-hour external prediction files differ below the fourth
decimal place, which is why the result tables rather than those files are used as
the reproducibility check. No reported quantity has been observed to move because
of it.

Two quantities in this thesis did move during its development, and neither
movement is attributable to that instability. The utility ceiling moved from
0.039 to its present \pcUtilityMaxGbtB{} and the model-class gap from 0.0129 to
\pcHSixAbsEst. That was not run-to-run noise. It was Protocol Amendment 8
correcting a defect: the boosted comparator's early stopping had been selecting
its number of boosting rounds against the temporal test set, so a
model-selection choice was being made on the rows the model was then scored on.
Removing that optimistic bias lowered GBT's discrimination, which moved every
GBT-derived quantity at once and widened H6's gap in the interpretable model's
favour. Section~\ref{sec:limitations} states the amendment and the direction of
its effect; naming a leakage correction as though it were benign
non-determinism would understate it. The two superseded figures are typed here
rather than generated, because they are the only quantities in this document
that must \emph{not} track the current run: replacing them with macros would
print the present values twice and destroy the comparison the sentence exists to
make.

The freeze therefore exists for a narrower reason than run-to-run drift, and it
is still a good one: a number is only meaningful when paired with the run that
produced it, and anything built outside the pipeline (a slide, a screencast, a
figure recalled in a viva) can drift from the document without anything in the
document detecting it. This document is therefore bound to one run, and that run is
identified here so the pairing can be checked rather than assumed:

\begin{center}
\fbox{\parbox{0.88\textwidth}{\centering\vspace{2pt}
\textbf{Frozen run}\\[4pt]
tag \texttt{\pcFrozenTag} \quad$\cdot$\quad commit \texttt{\pcFrozenCommit}\\[2pt]
\small constants generated \pcFrozenDate
\vspace{2pt}}}
\end{center}

Two repository states are involved and they answer different questions, so the
distinction is drawn here rather than left to be discovered. The commit named
above is the \textbf{analysis provenance}: it is the state of the pipeline
source that produced every quantity this document reports, and it is what to
re-run to reproduce them. The \textbf{document provenance} is a later commit on
the same branch. Prose was corrected after the analysis commit without
re-running the pipeline, so the submitted sources and this PDF are carried by
that later commit, and the release tag is placed on it. What makes the pairing
safe is that nothing regenerated a number in between: every edit after the
analysis commit was to prose, captions or document configuration, and none
touched a pipeline stage, a configuration value or a result table. That is a
checkable claim rather than an assurance, and two commands check it. A
\texttt{-dirty} suffix on the commit would indicate that the constants were
generated from an uncommitted working tree, and must not appear in a submitted
document.

\begin{verbatim}
git show <tag>:sepsis/thesis/generated/pipeline_constants.tex \
    | grep pcFrozenCommit
git diff --stat <analysis-commit> <tag> -- sepsis/project/
\end{verbatim}

\noindent where \texttt{<analysis-commit>} is the commit printed above and
\texttt{<tag>} the release tag. The first must print that same commit: every
number in this thesis is expanded from that one generated file, and the file
records which run wrote it. The second must print nothing, which is what
establishes that no pipeline source changed between the run and the submitted
document. Checking out the tag then yields the submitted sources and this PDF
alongside the code that produced its numbers. Re-deriving the file from the databases additionally
requires credentialed PhysioNet access, since no derived table is redistributed
here (Section~\ref{app:repo}); the reported quantities themselves are aggregate
results and are disclosed in this document rather than as data.

Everything produced after the freeze (the presentation, the screencast and
the viva preparation) reads from the frozen constants rather than from a
fresh run, so that no figure quoted aloud can disagree with the figure on the
page.
...
\begin{longtable}{p{4.6cm}p{8.4cm}}
    ...
    \endfoot
        ...
        Amendments 1--10, each dated and disclosed as post-hoc \\
        \addlinespace
        \texttt{config.R} &
        ...
        No pipeline configuration constant is defined in another analysis
        script \\
        \addlinespace
        \texttt{utils.R} &
...
stochastic step is seeded, so within the pinned analysis environment a repeated
run reproduces the reported estimates; the gradient-boosted comparator may vary
slightly across library builds or architectures, as noted below.
