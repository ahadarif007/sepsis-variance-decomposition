...
tables by \texttt{thesis\_constants.R}, so the document is always internally
consistent. Whether it is consistent across \emph{runs} is a separate question,
and it is worth answering with a measurement rather than an assumption.
...
One thing this section previously attributed to that instability should be
corrected, because the true explanation is less comfortable and is disclosed
elsewhere in this thesis. Between two drafts the utility ceiling moved from
...
The commit named above is the state of the pipeline source that produced these
numbers; the tag is placed on the commit that carries the rebuilt document, so
that checking out the tag yields both the code and this PDF together. A
...
\begin{verbatim}
git checkout <tag>
F=sepsis/thesis/generated/pipeline_constants.tex
diff <(git show <tag>:$F) $F
\end{verbatim}

\noindent Every number printed in this thesis is expanded from that one
generated file, so an empty diff establishes that the document and the tagged
repository state agree. Re-deriving the file from the databases additionally
