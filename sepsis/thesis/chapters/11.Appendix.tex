...
consistent. It is not, however, consistent across \emph{runs}: the
gradient-boosted comparator is not bit-stable, so re-executing the pipeline
moves every GBT-derived quantity slightly. Between two drafts of this work the
utility ceiling moved from 0.039 to its present \pcUtilityMaxGbtB{} and the
model-class gap from 0.0129 to \pcHSixAbsEst, for exactly this reason and with
no defect involved. The two superseded figures are typed here rather than
generated, because they are the only quantities in this document that must
\emph{not} track the current run: replacing them with macros would print the
present values twice and destroy the comparison the sentence exists to make.

The consequence is that a number is only meaningful when paired with the run
that produced it. This document is therefore bound to one run, and that run is
