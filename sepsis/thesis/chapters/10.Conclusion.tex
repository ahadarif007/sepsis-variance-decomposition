...
The answer is not the one the study anticipated. Across the six model
specifications compared here, model class dominates discrimination. External transport has the second-largest point estimate,
several times the label definition's, but its interval is too wide to establish
that rank or that it moves AUROC more than the label does. The label and the
split design move it least, and both are measured on the full test cohort. What the label determines is
...
The empirical question was answered instead by relabelling the same stays,
whose septic membership is cohort-wide identical,
...
treatment, separable from the surrounding at-risk hours at
\pcPreTreatAurocPrimaryF{} AUROC on the onset-hour estimand. A published MIMIC-IV
real-time sepsis model can evaluate prediction before clinical action only if
its onset rule allows a labelled onset to precede that action, which under a
conjunction rule it cannot. Demonstrating advance prediction requires more
again: that the predictors themselves precede the onset timestamp.

\paragraph{H4 (Split optimism): Not confirmed.}
The point estimate favours the temporal split, and the interval covers zero
while excluding the pre-registered 0.02 threshold, so the data do not support
the claim that a random split inflates AUROC by at least that much. For the primary model it scores
...
\paragraph{H5 (Metric divergence): Not evaluable as pre-registered.}
The hypothesis compares two proportional declines, and that contrast cannot be
formed here: there is no internal Utility Score for a decline to be
proportional to. What occupies H5's slot in the confirmatory family is a
substituted estimand, the AUROC limb alone, disclosed as such in the
pre-registration rather than reconstructed afterwards
(Section~\ref{sec:h5}). That substituted contrast is itself inconclusive, and
inconclusive in an informative way. External discrimination is
\pcExtAurocPrimaryB{} on the microbiology-covered sub-cohort of eICU-CRD,
the only stays in which the Sepsis-3 anchor can fire at all,
...
still covers zero, because the Sepsis-3 anchor is recoverable in only
\pcExtMicroCoverage\% of eICU-CRD stays and the estimate rests on
\pcExtNEventsB{} events. A better-powered external cohort would have resolved
this substituted contrast, though not the pre-registered hypothesis, which
remains unevaluable because its proportional utility contrast is undefined. The antibiotic-only arm (\pcExtAbxNEventsB{}
events, a different target) shows the same degradation with the same ordering
across labels, and the anchor-independent label added under Protocol
Amendment~9 shows a degradation of the same order
(\pcExtAltDropPrimary, 95\% CI \pcExtAltDropCiPrimary) on
\pcExtAltNEvents{} events across the whole of eICU-CRD, which is the one
external interval in this study that is not limited by its event count. Every external point estimate is below its
internal counterpart and all three arms agree on direction, but the primary contrast's interval covers
zero, so the study establishes neither the magnitude of the degradation nor,
on the Sepsis-3 arm alone, that there is one.
...
clinical utility give materially different assessments of the same models,
discrimination sitting in the high 0.7s while no threshold yields a usable
combination of net utility and alert burden. No correlation between the two
metrics is estimated here, and none is claimed.

\paragraph{H6 (Model-class null): Not rejected; one-sided non-inferiority, not equivalence.}
...
and a difference larger than the margin, in the interpretable model's favour,
remains consistent with these data. The interval also lies wholly below zero,
so the interpretable model is ahead rather than merely not behind, though by an
amount of the same order as the label spread. This is the strongest positive
...
well under a hundredth of a point on a scale bounded by 1, and both sit within
a few per cent of the no-alert strategy. The two metrics in fact order the pair
oppositely, the interpretable model ahead on discrimination and the boosted one
ahead on utility, which is the clearest illustration in the study of how little
either difference means: neither is large enough to choose on.


\section{The Central Lesson}

The results indicate that label membership and onset timing contribute to the
gap between discrimination and clinically usable alerting documented by Wang et
al.\ \cite{wang2025srpm}. They do not isolate the label as its sole cause:
the estimand, the alert threshold, calibration, prevalence and the Utility
Score's own penalties all bear on it, and this study varies only some of them.
What the label does contribute is specific and is set out below. The Sepsis-3
label on MIMIC-IV:
...
\begin{enumerate}
    ...
    onsets on the same stays, and an anchor-independent deterioration label
    ...
    label is not.
    \item \textbf{Produces unusable alert behaviour:} no evaluated operating
    point, under any pre-registered label variant, combines a material gain
    over the no-alert strategy with an alert burden near the pre-specified
    benchmark of 1.4 false alerts per true one. The best configuration under the primary label
    reaches \pcUtilityMaxGbtB{} at \pcUtilityOptBurdenGbtB{} false alerts per
    true one. The post-hoc variants reach higher ceilings, Variant D's being
    the highest, but they label different targets, and each still attains its
    own optimum at an alert burden several times the benchmark.
\end{enumerate}

These three problems are independent of model complexity. Improving
discrimination alone does not resolve the membership and timing limitations of
a label constituted by the very clinical actions it is supposed to precede;
addressing them requires changing the label.
...
sharpened because a label built from physiology alone, owing nothing to the
treatment anchor, agrees with Sepsis-3 barely better than chance
($\kappa = \pcKappaVsBD$). The disagreement is therefore not confined to
alternative readings of Sepsis-3: a post-hoc anchor-independent deterioration
construct selects substantially different stays at substantially different
hours.

The same comparison relocates the difficulty, and not where the conjunction
framing expected. The treatment anchor on its own is about as predictable as
Sepsis-3 built on top of it, so requiring both limbs is not what makes the
label hard. The strength of that statement depends on which model is asked. The
anchor-attributable fraction is \pcPhiAnchorPrimary{}
[\pcPhiAnchorCiLoPrimary, \pcPhiAnchorCiHiPrimary] for the primary model, an
interval that includes 1, and \pcPhiAnchorGbt{}
[\pcPhiAnchorCiLoGbt, \pcPhiAnchorCiHiGbt] for the gradient-boosted
comparator, an interval that does not. What both agree on is the sign and the
size: the anchor limb is no harder than the conjunction, and on the boosted
model it is slightly easier. Neither supports the reading the conjunction
framing predicted, that Sepsis-3 is harder than either of its limbs.

Sepsis-3 with a better clock is easier again (\pcAltAurocFullPrimaryF{} against
\pcAurocPrimaryB). Variant F holds cohort membership fixed and changes only the
onset timestamp, which is what makes that contrast interpretable. It is not
computed on an identical event set: moving an onset earlier can move it inside
the 72-hour horizon, so F carries \pcFNExtraInHorizon{} stays that conjunction
timing discards (Section~\ref{sec:pretreatment_label}). Variant D's higher figure is not offered in
support here: its discrimination level is set in part by its own construction
and is not comparable with Variant B's
(Section~\ref{sec:variant_d_establishes}); what D contributes is its
disagreement with Sepsis-3, not its AUROC. The difference in onset-hour discrimination is associated less with the
conjunction itself than with the timestamp assigned to it: the clinician's
action rather than the patient's deterioration.
...
\begin{enumerate}
    ...
    \item \textbf{C2 (Treatment leakage separated: membership from onset
    timing):} A proof that
    ...
    \pcPreTreatOnsetsVarF{} pre-treatment onsets on the same stays, whose septic
    membership it leaves cohort-wide unchanged,
    separable from their surrounding at-risk hours at
    \pcPreTreatAurocPrimaryF{} AUROC on the onset-hour estimand. This confirms Kamran et al.'s
    ...
    script, which within the pinned environment reproduces the reported
    estimates from the raw databases (the gradient-boosted comparator being the
    one output that may vary slightly across library builds, thread settings or
    architectures, and whose rows therefore serve as the designated consistency
    check on any re-run), and which emits the numbers in this document as generated
    ...
    An anchor-independent deterioration label, tested alongside the three
    ...
    comparison shows that the treatment anchor alone is about as predictable as
    Sepsis-3 built on top of it ($\phi_{\text{anchor}} = \pcPhiAnchorPrimary$
    [\pcPhiAnchorCiLoPrimary, \pcPhiAnchorCiHiPrimary] for the primary model,
    an interval including 1; $\pcPhiAnchorGbt$
    [\pcPhiAnchorCiLoGbt, \pcPhiAnchorCiHiGbt] for the boosted comparator,
    which excludes it), and that the pre-treatment window empty of Sepsis-3
    onsets contains
    \pcPreTreatOnsetsVarD{} anchor-independent deterioration onsets
    identified at \pcPreTreatAurocPrimaryD{} AUROC, and
    \pcPreTreatOnsetsVarF{} Sepsis-3 onsets once the standard onset timing is
    restored. Together these show that label membership and onset timing
    materially shape the prediction task as evaluated; they do not establish
    that the physiology itself is easy to predict, since Variant D's
    discrimination level is set in part by its own construction
...
class's \pcSpreadModelClass{}. Two of the four decomposition intervals cover
zero, the split contrast and the external-transport contrast, the latter with
much the wider interval. Of the
three pre-registered hypotheses that admitted a statistical test, H1 and H4
are not confirmed and both fail in the direction opposite to the one predicted,
while H6 supports one-sided non-inferiority but not two-sided equivalence. H2
was evaluated descriptively, H3 and H5 were not evaluable as pre-registered,
and the contrast substituted for H5 is inconclusive. That outcome is reported here in the
...
fix what the answer means before the answer is known, and they separate an
effect that is small from one that is not there at all, which is the
distinction on which two of these four verdicts turn.
...
which patients are septic, and through onset timing, which fixes whether a
real-time model can be asked to predict anything ahead of clinical action at
all. Those are the channels the reported
...
variants. The pre-treatment window is not empty of recorded deterioration: it
contains \pcPreTreatOnsetsVarD{} onsets under an anchor-independent label and
\pcPreTreatOnsetsVarF{} under Sepsis-3 itself, once the onset is timed as
$\min(t_{\text{susp}}, t_{\text{SOFA}})$ rather than as the suspicion time
alone. Those onsets are separable from the surrounding at-risk hours
(\pcPreTreatAurocPrimaryD{} and \pcPreTreatAurocPrimaryF{} AUROC), but that is
onset-hour discrimination and may draw on measurements charted later within the
same hourly bin, so it establishes that the events exist and are representable,
not that they can be predicted in advance. The window was empty here because of
an onset rule, and that rule is not peculiar to this thesis. The data contain
information associated with the recorded onset hour; whether it supports
actionable advance warning is a question a lagged or sub-hour design would have
to answer.
