...
\begin{enumerate}
    ...
    label is not.
    \item \textbf{Produces unusable alert behaviour:} no model, under any
    pre-registered label variant, has a threshold at which it both beats the
    no-alert strategy by a clinically meaningful margin and raises an alarm
    volume a ward could absorb. The best configuration under the primary label
...
disagreement with Sepsis-3, not its AUROC. What resists prediction is not the
conjunction but
